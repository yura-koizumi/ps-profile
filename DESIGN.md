...
リモート/ローカル自動判定: スクリプト隣接に `modules/PSProfile/PSProfile.psm1` があればローカルコピー、無ければ GitHub raw から取得。
リモート取得時は `cacheBust=<timestamp>` を付与し、CDN / proxy キャッシュの影響を避ける。
...
VSCode / Git / npm / pip の global proxy は、社内ネットワーク向けにユーザーが意図して設定している可能性があるため install/update/uninstall では自動削除しない。ただし `px-state` / `px-doctor` で現在値を表示し、現セッション proxy と矛盾する場合は解除コマンドを推奨アクションとして表示する。
...
- `install.ps1` 全面書き直し: `-Update` / `-Uninstall` / `-SkipDeps` / `-Branch`、GitHub raw 経由リモート対応
